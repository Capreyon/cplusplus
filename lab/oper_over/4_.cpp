...
istream & operator >> (istream &input, polynomial &in)
{
	cout<<"enter the data of polynomial   : \n";
	while(1)
	{
		int cf,power;
		input >> cf >> power;
...
ostream & operator << (ostream &output, polynomial &out)
{
		struct poly*head3 = out.head;
		while(head3)
		{
				{	
					if(head3->next)
					output << head3->cf <<  "x^" << head3->power << " + " ;
					else
					output<< head3->cf <<  "x^" << head3->power;
					head3 = head3->next;
...
int main()
{
	...
	p3 = p1+p2;
	p4 = p1*p2;
	cout<<endl<<"addition of polynomial : ";
	cout<<p3;
